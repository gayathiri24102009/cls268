...
def oil_effect(decode_array_to_img): #267
    oil_effect_img = cv2.x_photo.oilPainting(decode_array_to_img, 7, 1)
...
def water(decode_array_to_img):
    waterimg=cv2.stylization(decode_array_to_img,sigma_s=20,sigma_r=0.6)
    status,output_image=cv2.imencode(".PNG",waterimg)

    return output_image

def invert(decode_array_to_img):
    invertimg=cv2.bitwise_not(decode_array_to_img)
    status,output_image=cv2.imencode(".PNG",invertimg)

    return output_image

def hdr(decode_array_to_img):
    hdrimg=cv2.detailEnhance(decode_array_to_img,sigma_s=12,sigma_r=0.15)
    status,output_image=cv2.imencode(".PNG",hdrimg)
